...
namespace PartyInc
{
    namespace KK
    {
        public class Mono_FinishScreenUI_KK : MonoBehaviour
        {
            ...
            private void OnEnable()
            {
                ...
                for (int i = 0; i < Mng_GameManager_KK.Current.playerCount; i++)
                {
                    players[i].SetActive(true);
                }

                if (Mng_GameManager_KK.Current.playerCount == 1)
                {
                    highScores.SetActive(true);
                    highScore.text = PlayerPrefs.GetInt(PartyInc.Constants.KK_KEY_HISCORE).ToString();

                    playerNames[0].text = PhotonNetwork.LocalPlayer.NickName;
                    playerPlacings[0].enabled = false;
                    playerScores[0].text = Mng_GameManager_KK.Current.playerResults[0].scoring.ToString();

                    if (Mng_GameManager_KK.Current.IsHighScore)
                    {
                ...
                else
                {
                    ...
                    // Set flavour text
                    if (Mng_GameManager_KK.Current.WinnerId == -1)
                    {
                        flavourTitle.text = "Unbelievable! Its a draw!";
                    }
                    else
                    {
                        if (Mng_GameManager_KK.Current.WinnerId == PhotonNetwork.LocalPlayer.ActorNumber)
                        {
            ...
            {
                var aux = Mng_GameManager_KK.Current.playerResults.OrderByDescending(r => r.scoring);
