...
namespace PartyInc
{
    namespace AS
    {
        public class Mono_FinishScreenUI_AS : MonoBehaviour
        {
            ...
            private void OnEnable()
            {
                ...
                for (int i = 0; i < Mng_GameManager_AS.Current.playerCount; i++)
                {
                    players[i].SetActive(true);
                }

                if (Mng_GameManager_AS.Current.playerCount == 1)
                {
                    ...
                    playerScores[0].text = Mng_GameManager_AS.Current.playerResults[0].scoring.ToString();
                ...
                else
                {
                    ...
                    // Set flavour text
                    if (Mng_GameManager_AS.Current.WinnerId == -1)
                    {
                        flavourTitle.text = "Unbelievable! Its a draw!";
                    }
                    else
                    {
                        if (Mng_GameManager_AS.Current.WinnerId == PhotonNetwork.LocalPlayer.ActorNumber)
                        {
            ...
            {
                var aux = Mng_GameManager_AS.Current.playerResults.OrderByDescending(r => r.scoring);
