...
namespace PartyInc
{
    namespace AS
    {
        public class Mono_ProceduralGenerator_AS : MonoSingleton<Mono_ProceduralGenerator_AS>
        {
            ...
            {
                base.Init();
                Mono_ObstacleDeathCount_AS.onObstacleDied += SpawnObstacle;
            }
            ...
            private void Update()
            {
                ...
                {
                    _currentSeparation = _startingSeparation + Mng_GameManager_AS.Current.InGameTime * ((_finalSeparation - _startingSeparation) / _timeToReachFinalSeparationInSeconds);
                }
            }
            ...
            {
                SpawnObstacle();
                _newest.GetComponent<Mono_ObstacleDeathCount_AS>().CurrentDeathCount = deathCount;
            }

            private IEnumerator ObstaclesBaseCase()
            {
                yield return new WaitUntil(() => Mng_GameManager_AS.Current.MyPlayerZ != -1);
            ...
            private void SpawnObstacle()
            {
                ...
                _newest = Mng_ObstaclePoolManager_AS.Current.RequestObstacle();
                _newest.transform.position = new Vector3(_x, y, Mng_GameManager_AS.Current.MyPlayerZ);
                ...
                _newest.GetComponent<Mono_Obstacle_AS>().SetObstaclesOpening(diff);
                _x += 7f;
            }

            private IEnumerator WaitForSeed()
            {
                yield return new WaitUntil(() => Mng_GameManager_AS.Current.GotSeed);
