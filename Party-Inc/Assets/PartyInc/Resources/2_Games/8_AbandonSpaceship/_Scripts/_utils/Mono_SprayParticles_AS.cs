...
namespace PartyInc
{
    namespace AS
    {
        public class Mono_SprayParticles_AS : MonoBehaviourPun, IPunInstantiateMagicCallback
        {
            ...
            public void OnPhotonInstantiate(PhotonMessageInfo info)
            {
                ...
                foreach(PhotonView pv in FindObjectsOfType<PhotonView>())
                {
                    if(pv.OwnerActorNr == info.Sender.ActorNumber && pv.gameObject.tag == "Player")
                    {
                        ...
                        {
                            // bottom
                            myPlayer = pv.GetComponent<Mono_Player_Controller_AS>().Bottom.transform;
                        }
                        else
                        {
                            // top
                            myPlayer = pv.GetComponent<Mono_Player_Controller_AS>().Top.transform;
                        }
                    }
                }
