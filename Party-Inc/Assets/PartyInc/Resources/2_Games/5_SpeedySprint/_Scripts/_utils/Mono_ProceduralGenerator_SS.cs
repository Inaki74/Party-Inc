...
namespace PartyInc
{
    namespace SS
    {
        ...
        /// </summary>
        public class Mono_ProceduralGenerator_SS : MonoBehaviourPunCallbacks
        {
            ...
            void Update()
            {
                if(!_started && Mng_GameManager_SS.Current.GameBegan)
                {
                    _started = true;
                ...
                _subSectionTest = Mng_GameManager_SS.Current.Testing;
            }

            private void Awake()
            {
                Mono_InvisibleTrolley_SS.onPassedSection += GenerateNextSection;
                Mng_UIManager_SS.onInputDone += OnInputDone;

                // We decide where to put the sub-sections (dependent on Mng_GameManager_SS's PlayerCount).
            ...
            {
                switch (Mng_GameManager_SS.Current.playerCount)
                {
                    case 1:
            ...
            {
                Object[] all = Resources.LoadAll(Mng_GameManager_SS.SubsectionsPath + difficulty, typeof(GameObject)).Cast<GameObject>().ToArray();
            ...
            private void PlaceSection(int[] section)
            {
                ...
                if (Mng_GameManager_SS.Current.playerCount > 1 && PhotonNetwork.IsMasterClient)
                {
            ...
            {
                GameObject toPlace = Mng_SubSectionSpawner_SS.Current.RequestSubsection(diff, subsec);
                
                toPlace.transform.position = new Vector3(spawner.transform.position.x, 0.1f, _currentZ);

                return toPlace.GetComponent<Mono_SubSection_SS>().AmountOfTiles;
            }
