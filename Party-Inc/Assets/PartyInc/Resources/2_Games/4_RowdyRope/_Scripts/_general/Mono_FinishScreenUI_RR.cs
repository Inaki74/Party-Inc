...
namespace PartyInc
{
    namespace RR
    {
        public class Mono_FinishScreenUI_RR : MonoBehaviour
        {
            ...
            private void OnEnable()
            {
                ...
                for (int i = 0; i < Mng_GameManager_RR.Current.playerCount; i++)
                {
                    players[i].SetActive(true);
                }

                if (Mng_GameManager_RR.Current.playerCount == 1)
                {
                    highScores.SetActive(true);
                    highScore.text = PlayerPrefs.GetInt(PartyInc.Constants.RR_KEY_HISCORE).ToString();

                    playerNames[0].text = PhotonNetwork.LocalPlayer.NickName;
                    playerPlacings[0].enabled = false;
                    playerScores[0].text = Mng_GameManager_RR.Current.playerResults[0].scoring.ToString();

                    if (Mng_GameManager_RR.Current.IsHighScore)
                    {
                ...
                else
                {
                    ...
                    // Set flavour text
                    if (Mng_GameManager_RR.Current.WinnerId == -1)
                    {
                        flavourTitle.text = "Unbelievable! Its a draw!";
                    }
                    else
                    {
                        if (Mng_GameManager_RR.Current.WinnerId == PhotonNetwork.LocalPlayer.ActorNumber)
                        {
            ...
            {
                var aux = Mng_GameManager_RR.Current.playerResults.OrderByDescending(r => r.scoring);
