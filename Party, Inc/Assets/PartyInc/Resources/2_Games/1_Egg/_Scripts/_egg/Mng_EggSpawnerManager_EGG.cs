...
namespace PartyInc
{
    namespace EGG
    {
        ...
        /// </summary>
        public class Mng_EggSpawnerManager_EGG : MonoBehaviourPun
        {
            ...
            private List<Mono_EggSpawner_EGG> eggSpawners = new List<Mono_EggSpawner_EGG>();
            #endregion
            ...
            private void Update()
            {
                if (!Mng_GameManager_EGG.Current.GameBegan) return;
                ...
                if (!editorMode)
                {
                    ...
                    if (currentTime - startTime > timeLimit)
                    {
                        ...
                        if (currentWave >= 10)
                        {
                            ...
                            if (runOnce)
                            {
                                if (PhotonNetwork.IsMasterClient) Mng_GameManager_EGG.Current.SetGameFinished(true);
                                runOnce = false;
                            }
                            return;
                        }
            ...
            {
                switch (Mng_GameManager_EGG.Current.playerCount)
                {
                    case 1:
            ...
            private void SetUpSpawners()
            {
                ...
                foreach (Transform go in spawnerPositions)
                {
                    ...
                    Mono_EggSpawner_EGG sp = spGo.GetComponent<Mono_EggSpawner_EGG>();
                    sp.SetIntervals(waveIntervals);
                    // THis is to make that eggs spawning from certain spawners appear as "theirs"
                    if (p % Mng_GameManager_EGG.Current.playerCount == PhotonNetwork.LocalPlayer.ActorNumber - 1)
                    {
                        sp.IsMine = true;
                    }
                    eggSpawners.Add(sp);
            ...
            /// </summary>
            private void UpdateRoutines(Mono_EggSpawner_EGG spawner, List<int> newRoutine)
            {
            ...
            private void UpdateAllSpawners(int[,] map)
            {
                ...
                // Load all Spawners
                int c = 0;
                foreach(Mono_EggSpawner_EGG s in eggSpawners)
                {
            ...
            {
                eggMap = Mng_GameManager_EGG.Current.GetEggMap(round);
